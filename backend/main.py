...
async def callback_hubspot_integration(request: Request):
    return await oauth2callback_hubspot(request)

@app.post('/integrations/hubspot/items')
async def hubspot_items_integration(credentials: str = Form(...)):
    return await get_items_hubspot(credentials)


@app.get("/test/hubspot/items")
async def test_hubspot_items(user_id: str = "TestUser", org_id: str = "TestOrg"):
    credentials = await get_hubspot_credentials(user_id, org_id)
    items = await get_items_hubspot(json.dumps(credentials))
    return items